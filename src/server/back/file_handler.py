...
class FileHandler():
        ...
        for root, dirs, files in os.walk(f"{cfg.VIRT_DATA_FOLDER}"):
            for file in files:
                if data_group in file:
                    data = self.__get_dict_from_json_file(os.path.join(root, file))
                    if data_group in self.__file_data:
                        self.__file_data[data_group] = self.__file_data[data_group] + data
                    else:
    ...
    def prepare_vm_environment(self, excel_file: str) -> None:
        ...
        with open(
            f"{cfg.BACK_FILES_FOLDER}/bash_commands.sh", 'a',
            encoding="utf-8"
        ) as file:
