...
class CyberbackupHelper():
    ...
    def __init__(self, dpc: str, logger: Logger=Logger()):
        ...
        self.__dpc = dpc
        db_password = os.getenv("CB_DB_PASS")
    ...
    #         upstert_uuid = uuid.uuid5(namespace, f"{row[0]}-{row[2]}-{row[3]}")
    #         db_el = Backups(
    #             name=row[0],
    #             uuid=upstert_uuid,
    ...
    #     aggregator_db = DBManager(env="dev")
    #     aggregator_db.add_data(data)
    #     aggregator_db.close()
